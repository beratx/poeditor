...
//			console.groupEnd();
     ...
     *@param {Array} bounds - the bounds of the rectangle defined by (x1, y1, x2, y2)
     *@param {Boolean} closedPolyline - incase polyline is closed component then true, else false
     * 
     *@return true - if line intersects the rectangle, false - if not
     *@author Alex Gheorghiu <alex@scriptoid.com>
    ...
    polylineIntersectsRectangle:function(points, bounds, closedPolyline){
        ...
        //check the closed component - that is last point connected to the first
        if (closedPolyline){
            //create a line out of each 2 consecutive points            
    ...
    getEndPoint:function(startPoint, length, angle){
        ...
        endPoint.transform(Matrix.generateTranslationMatrix(-startPoint.x, -startPoint.y));
        endPoint.y -= length;
        //endPoint.transform(Matrix.rotationMatrix(angle));
        endPoint.transform(Matrix.generateRotationMatrix(angle));
        //endPoint.transform(Matrix.translationMatrix(startPoint.x, startPoint.y));
        endPoint.transform(Matrix.generateTranslationMatrix(startPoint.x, startPoint.y));
        return endPoint;
    },
     ...
     * it can be one of: Component, Group, Connector, Container or none.
     * 
     * Note: Connectors are more important than Components and Components more important 
     * than Container so Connectors > Components > Container
    ...
    getObjectByXY: function(x, y){
        ...
        var cId = Stack.getComponentByCoord(x, y);
        if(cId != -1){ // found a Component
            var gId = Stack.getComponentById(cId).groupId;
                if(gId != -1){ // if the Component belongs to a Group then return that Group
                    return {
                        id: gId,
                        type: 'Group'
                    }
                }
                else{ // lonely Component
                    return {
                        id: cId,
                        type: 'Component'
                    };
