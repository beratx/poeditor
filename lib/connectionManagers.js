...
ConnectorManager.prototype = {
    ...
    connectorPaint:function(context, highlightedConnectorId){
        //console.log("connectorPaint: id " + highlightedConnectorId);
        for(var i=0; i<this.connectors.length; i++){
            //PAINT A CONNECTOR
            ...
            if(this.connectors[i].id == highlightedConnectorId){
                ...
                context.save();
                context.lineWidth = defaultLineWidth + 2;
...
//            }
    ...
    connectorConnect:function(connectionPoint1, connectionPoint2){
        ...
        var figurePoint;
        var nonComponentPoint;

        //find out which one is from a connector
        //set the connectors point to the figures point, has a "snap to" effect
        if(connectionPoint1.type == ConnectionPoint.TYPE_COMPONENT){
            figurePoint = connectionPoint1;
            nonComponentPoint = connectionPoint2;
        }
        else{
            figurePoint = connectionPoint2;
            nonComponentPoint = connectionPoint1;
        }

        //make connector's point be identical with Component's point
        nonComponentPoint.point.x = figurePoint.point.x;
        nonComponentPoint.point.y = figurePoint.point.y;

        //are these connectionPoints already connected, if not connect them now?
        if(!this.connectionPointIsConnected(nonComponentPoint.id,figurePoint.id)){
            this.glueCreate(nonComponentPoint.id,figurePoint.id, false);
        }
        
        //if we are now connected at two places, make the line jagged.
        var connector = this.connectorGetById(nonComponentPoint.parentId);
    ...
    connectorAdjustByConnectionPoint: function(cpId, x, y){
        ...
        if(con.type === Connector.TYPE_STRAIGHT){
            ...
            var sComponent = Stack.figureGetAsFirstComponentForConnector(con.id);
            var sBounds =  sComponent == null ? null : sComponent.getBounds();

            //second bounds (of end figure)
            var eComponent = Stack.figureGetAsSecondComponentForConnector(con.id);
            var eBounds = eComponent == null ? null : eComponent.getBounds();
     ...
     *@param {Number} startComponentId - Start {Component}'s id or -1 if no start {Component} present
     *@param {Point} startPoint - Start {Point} of connection
     *@param {Number} endComponentId - End {Component}'s id or -1 if no start {Component} present
     ...
     *  startPoint, 
     *  endPoint, 
     *  start {Component}'s {ConnectionPoint} Id - or -1 if none, 
     *  end {Component}'s {ConnectionPoint}'s Id - or -1 if none
     *  ]
     *@author Artyom Pokatilov <artyom.pokatilov@gmail.com>
     **/
    getClosestPointsOfConnection: function(startAutomatic, endAutomatic, startComponentId, startPoint, endComponentId, endPoint) {
        ...
        if (startAutomatic && !endAutomatic) {
            return this.closestPointsAuto2Fixed(startComponentId, endPoint);
        }
        ...
        if (!startAutomatic && endAutomatic) {
            return this.closestPointsFixed2Auto(startPoint, endComponentId);
        }
        ...
        if (startAutomatic && endAutomatic) {
            return this.closestPointsAuto2AutoB(startComponentId, startPoint, endComponentId, endPoint);
        }
    },
    
    getClosestPointToConnector : function(componentId, point){
        var candidateCps = this.connectionPointGetAllByParent(componentId),
            candidateCpsLength = candidateCps.length,
            curCp,
            closestCp = candidateCps[0].point,
            closestCpId = candidateCps[0].id,
            minDistance = Util.distance(closestCp, point),
            curDistance;            
        

        // find closest to endPoint
        for(var i = 0; i < candidateCpsLength; i++){
            curCp = candidateCps[i].point;
            curDistance = Util.distance(curCp, point);
            
            if (curDistance < minDistance) {
                minDistance = curDistance;

                closestCp = curCp;
                closestCpId = candidateCps[i].id;
            }            
            
        }
        
        return [closestCp.clone(), closestCpId];
        
    },
    
    closestPointsAuto2AutoB : function(startComponentId, startPoint, endComponentId, endPoint){
        ...
        /*if (startComponentId == endComponentId) {
            ...
            var candidate = this.closestPointsAuto2Fixed(
                startComponentId,  //start figure's id
                startPoint, //start point
                endComponentId, //end figure's id
                endPoint //end 
            );
        ...
        var startFCps = this.connectionPointGetAllByParent(startComponentId),
            startFCpLength = startFCps.length,
            curStartPoint,
            closestStartPoint = startFCps[0].point,
            closestStartConnectionPointId = startFCps[0].id,
            endFCps = this.connectionPointGetAllByParent(endComponentId),
     ...
     * endAutomatic = true
     * */
    closestPointsAuto2Auto : function(startComponentId, startPoint, endComponentId, endPoint){
        // special case when figure is connected to itself
        if (startComponentId == endComponentId) {
            // we will find closest to end point (mouse coordinates)
            // this means solutions will be the same as connecting to end point
     ...
     * endAutomatic = true
     * */
    closestPointsFixed2Auto : function(startPoint, endComponentId){
        //get all connection points of end figure
        var cCps = this.connectionPointGetAllByParent(endComponentId);
        var cCpLength = cCps.length;
        //console.log("closestPointFixed2Auto: " + state);
        //console.log("cCps[0].point.x: " + cCps[0].point.x);
        var closestPoint = cCps[0].point;
        var closestConnectionPointId = cCps[0].id;
        var minDistance = Util.distance(startPoint, closestPoint);
        var curPoint;
        var curDistance;

            
        // find closest to startPoint
        for(var i = 1; i < cCpLength; i++){
            curPoint = cCps[i].point;
            curDistance = Util.distance(startPoint, curPoint);
            if (curDistance < minDistance) {
                minDistance = curDistance;
                closestPoint = curPoint;
                closestConnectionPointId = cCps[i].id;
            }
        }
     ...
     * endAutomatic = false
     * */
    closestPointsAuto2Fixed: function(startComponentId, endPoint){
        //get all connection points of start figure
        var cCps = this.connectionPointGetAllByParent(startComponentId),
            cCpLength = cCps.length,
            closestPoint = cCps[0].point,
            closestConnectionPointId = cCps[0].id,
     ...
     *@param {Array} sBounds - the starting bounds (of a Component) as [left, top, right, bottom] - area we should avoid
     *@param {Array} eBounds - the ending bounds (of a Component) as [left, top, right, bottom] - area we shoudl avoid
     *
    ...
    connector2Points: function(type,  startPoint, endPoint, sBounds, eBounds ){
        ...
        //console.log("ConnectionManager: connector2Points (" + type + ", " + startPoint + ", " + endPoint + ", " + sBounds + ", " + eBounds + ')');
        var solutions = [];
        ...
        if(sBounds != null){
            ...
            potentialExits.push(new Point(startPoint.x, sBounds[1] - COMPONENT_ESCAPE_DISTANCE)); //north
            potentialExits.push(new Point(sBounds[2] + COMPONENT_ESCAPE_DISTANCE, startPoint.y)); //east
            potentialExits.push(new Point(startPoint.x, sBounds[3] + COMPONENT_ESCAPE_DISTANCE)); //south
            potentialExits.push(new Point(sBounds[0] - COMPONENT_ESCAPE_DISTANCE, startPoint.y)); //west
        ...
        if(eBounds != null){
            ...
            potentialExits.push(new Point(endPoint.x, eBounds[1] - COMPONENT_ESCAPE_DISTANCE)); //north
            potentialExits.push(new Point(eBounds[2] + COMPONENT_ESCAPE_DISTANCE, endPoint.y)); //east
            potentialExits.push(new Point(endPoint.x, eBounds[3] + COMPONENT_ESCAPE_DISTANCE)); //south
            potentialExits.push(new Point(eBounds[0] - COMPONENT_ESCAPE_DISTANCE, endPoint.y)); //west
        ...
        if(true){ 
            ...
            if(startPoint.equals(endPoint)){
                //console.log("Start and end point coincide...skip backward solution. I think we will just fall on s0 :)");
            }
            else{
                //console.log("Filter backward solutions. Initial number of solutions = " + solutions.length);
        ...
        if(true){
            //cog.info("Get first class of solutions with same nr of points");
            if(solutions.length == 0){
     ...
     *  or 'figure' (ConnectionPoint.TYPE_COMPONENT)
     ...
     * 
     * @param {Number} parentId - the id of the parent ( Component or Connector )this ConnectionPoint will belong to
     * @param {Point} point - the location
     * @param {String} type - the type of parent. It can be 'figure' (ConnectionPoint.TYPE_COMPONENT)
    ...
    /** Removes the connectionPoints associated with a parent (it can be either Component or Connector)     
    ...
    /** Returns a subset of whole ConnectionPoints that belong to a figure or a connector
     *@param {Number} parentId - the figure or connector's id whom subset we want
     *@param {String} type - ConnectionPoint.TYPE_COMPONENT or ConnectionPoint.TYPE_CONNECTOR. No more guessing.
     *@return {Array}{ConnectionPoint}s
     *@author Alex Gheorghiu <alex@scriptoid.com>
     */
     ...
     * @param {Number} parentComponentId - the subset to obtain (optional)
     * If the parentComponentId is null we will get extend our search to all the ConnectionPoints on the canvas
     * If the parentComponentId is positive we will limit our search to that shape (Component or Connector)
     * If the parentComponentId is negative we will search on all Canvas but except that shape (Component or Connector)
     * @return {ConnectionPoint} that fit the criteria or null if none present
     * @author Alex Gheorghiu <alex@scriptoid.com>
     */
    connectionPointOver:function(x, y, parentComponentId){
        var foundedConnectionPoint = null;

        if(typeof(parentComponentId) == 'number'){ //we have a Component id specified
            if(parentComponentId < 0 ){ //search whole canvas except a figure
                for(var canvasConnectionPoint in this.connectionPoints){
                    if(this.connectionPoints[canvasConnectionPoint].parentId != -parentComponentId && this.connectionPoints[canvasConnectionPoint].contains(x,y)){
                ...
                var figureConnectionPoints = this.connectionPointGetAllByParent(parentComponentId);
    ...
    /**Paints ALL ConnectionPoints that are attached to a shape (Component or Connector)
     *@param {Context} context - the HTML5 canvas' context
     *@param {Number} parentComponentId - the the parent figure's ID
     */
    connectionPointPaint:function(context, parentComponentId){
        var figureConnectionPoints = this.connectionPointGetAllByParent(parentComponentId);
    ...
    /**
     * Transform all ConnectionPoints of a  Component
     * @param {Number} fId - the the parent figure's ID
     * @param {Matrix} matrix - the transformation matrix
     **/
    connectionPointTransform:function(cId, matrix){
        var cCps = this.connectionPointGetAllByParent(cId);
        var currentComponent = Stack.getComponentById(cId);
        ...
        for(var i = 0 ; i < cCps.length; i++){
            ...
            var glues = this.glueGetByFirstConnectionPointId(cCps[i].id);
            var gluesLength = glues.length;
            //Log.info("\tConnectionManager: connectionPointTransform()" + cCps[i].id + " glues = " + gluesLength);
            for(var j = 0; j < gluesLength; j++){
                ...
                // if current Component is glued with startPoint of the Connector
                if (firstCP.id == cCPs[0].id) {
                    // ConnectionPoint connected with moved (transformed) Component must be moved (transformed) as well
                    cCPs[0].transform(matrix);
                    
                    // in this case current Component is startComponent of the connection
                    ...
                    // get Component's ConnectionPoint which is glued with second ConnectionPoint of the Connector
                    var endComponentCP = endGlue ? this.connectionPointGetById(endGlue.id1) : null;
                    
                    // get endComponent as parent of endComponentCP
                    endComponent = endComponentCP ? Stack.getComponentById(endComponentCP.parentId) : null;
                ...
                } else {    // if current Component is glued with endPoint of the Connector
                    // ConnectionPoint connected with moved (transformed) Component must be moved (transformed) as well
                    cCPs[1].transform(matrix);
                    
                    // in this case current Component is endComponent of the connection
                    ...
                    // get Component's ConnectionPoint which is glued with first ConnectionPoint of the Connector
                    var startComponentCP = startGlue ? this.connectionPointGetById(startGlue.id1) : null;
                    
                    // get startComponent as parent of startComponentCP
                    startComponent = startComponentCP ? Stack.getComponentById(startComponentCP.parentId) : null;
                ...
                startBounds = startComponent ? startComponent.getBounds(): null;
                endBounds = endComponent ? endComponent.getBounds() : null;
                ...
                var candidate = CONNECTOR_MANAGER.getClosestPointsOfConnection(
                    ...
                    startComponent ? startComponent.id : -1, //start figure's id
                    startPoint, //start point
                    endComponent ? endComponent.id : -1, //end figure's id
                    endPoint //end point
                );

                //solutions
                CONNECTOR_MANAGER.debugSolutions = CONNECTOR_MANAGER.connector2Points(connector.type, candidate[0], candidate[1], startBounds, endBounds);

            
                // apply solution to Connector
                connector.applySolution(CONNECTOR_MANAGER.debugSolutions);
...
//                this.connectorAdjustByConnectionPoint(glues[j].id2 /*, x, y*/);
        ...
        var automaticGlues = this.glueGetByComponentId(fId);
     ...
     *@param {Number} pointId - {Component}'s {ConnectionPoint} id
     *@return {Array}{Glue}s
     *@author Alex Gheorghiu <alex@scriptoid.com>
     */
     ...
     *@return {Array}{Glue}s
     *@author Alex Gheorghiu <alex@scriptoid.com>
     *TODO: as second id is usually connector AND a Connector can not be connected to more than one Component
     ...
     *
     *@param {Number} firstId - the id of the first {ConnectionPoint} - usually the {Component}'s {ConnectionPoint} id
     ...
     *@param {Number} id1 - the id of the first shape (usually the Component)
    ...
    /**Removes all the  {Glue}s  based on first Id (usually the Component)
     *@param {Number} id - the id of the first shape (usually the Component)
     *@author Alex Gheorghiu <alex@scriptoid.com>
     **/
    ...
    /** Returns all {Glue}s that have the first Id equals with a certain id value
     *@param {Number} firstId - first id (usually {Component}'s id)
